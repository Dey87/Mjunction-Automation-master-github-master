...
public class TC_Auction_49_verifyDataofBidTrailReport extends BaseClass_Web {
	...
	@Test(description = "TC_Auction_49_verifyDataofBidTrailReport")
	public void verifyReportGenerationandViewReport(String no) throws Throwable {
		...
		auctioncomponentobj.ScheduleTheEvent(1, 2, 9);
		...
		auctioncomponentobj.NavigaterToAuctionParcipitation();
		auctioncomponentobj.Activate_Aution(AuctionDate);
		auctioncomponentobj.ParticipateInLiveAuctionTCS(decvalue, 1);
		etendercomponentobj.tenderLogout();
		...
		auctioncomponentobj.searchTheTenderRefNumber();
		auctioncomponentobj.BidTrailReport(otherFolder);
		auctioncomponentobj.VerofyDOwnloadedBIdTrailReport(otherFolder);
		auctioncomponentobj.logoutTheAuction();
	}
